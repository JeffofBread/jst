...
void selnotify(XEvent *e)
{
	...
	do {
		if (XGetWindowProperty(xw.dpy, xw.win, property, ofs, BUFSIZ / 4, False, AnyPropertyType, &type,
...
void xloadcols(void)
{
	...
	if (loaded) {
		for (cp = dc.col; cp < &dc.col[dc.collen]; ++cp) XftColorFree(xw.dpy, xw.vis, xw.cmap, cp);
	} else {
...
{
	if (!BETWEEN(x, 0, dc.collen - 1)) return 1;
...
int ximopen(Display *dpy)
{
	...
	if (xw.ime.xic == NULL) {
		xw.ime.xic = XCreateIC(xw.ime.xim, XNInputStyle, XIMPreeditNothing | XIMStatusNothing, XNClientWindow,
...
{
	if (ximopen(dpy)) XUnregisterIMInstantiateCallback(xw.dpy, NULL, NULL, NULL, ximinstantiate, NULL);
}
...
void xinit(int cols, int rows)
{
	...
	xw.attrs.event_mask = FocusChangeMask | KeyPressMask | KeyReleaseMask | ExposureMask | VisibilityChangeMask |
			      StructureNotifyMask | ButtonMotionMask | ButtonPressMask | ButtonReleaseMask;
	xw.attrs.colormap = xw.cmap;

	if (!(opt_embed && (parent = strtol(opt_embed, NULL, 0)))) parent = XRootWindow(xw.dpy, xw.scr);
	xw.win =
		XCreateWindow(xw.dpy, parent, xw.l, xw.t, win.w, win.h, 0, XDefaultDepth(xw.dpy, xw.scr), InputOutput,
...
void xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len, int x, int y)
{
	...
	if ((base.mode & ATTR_BOLD_FAINT) == ATTR_BOLD && BETWEEN(base.fg, 0, 7)) fg = &dc.col[base.fg + 8];
	...
	if (winy + win.ch >= borderpx + win.th) xclear(winx, winy + win.ch, winx + width, win.h);
	...
	if (IS_SET(MODE_FOCUSED)) {
		...
		case 4: /* Steady Underline */
			XftDrawRect(xw.draw, &drawcol, borderpx + cx * win.cw,
				    borderpx + (cy + 1) * win.ch - cursorthickness, win.cw, cursorthickness);
			break;
		case 5: /* Blinking bar */
		case 6: /* Steady bar */
			XftDrawRect(xw.draw, &drawcol, borderpx + cx * win.cw, borderpx + cy * win.ch, cursorthickness,
				    win.ch);
			break;
		}
	} else {
...
{
	if (!BETWEEN(cursor, 0, 7)) /* 7: jst extension */
		return 1;
	win.cursor = cursor;
	return 0;
}
...
run:
	...
	if (!opt_title) opt_title = (opt_line || !opt_cmd) ? "jst" : opt_cmd[0];
