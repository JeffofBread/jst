...
void xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len, int x, int y)
{
	...
	if (y == 0) xclear(winx, 0, winx + width, win.vborderpx);
	if (winy + win.ch >= win.vborderpx + win.th) xclear(winx, winy + win.ch, winx + width, win.h);
	...
	if (IS_SET(MODE_FOCUSED)) {
		...
		case 4: /* Steady Underline */
			XftDrawRect(xw.draw, &drawcol, win.hborderpx + cx * win.cw,
				    win.vborderpx + (cy + 1) * win.ch - cursorthickness, win.cw, cursorthickness);
			break;
		case 5: /* Blinking bar */
		case 6: /* Steady bar */
