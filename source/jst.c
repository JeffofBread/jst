...
{
	struct timespec now;
	if (su && !clock_gettime(CLOCK_MONOTONIC, &now) && TIMEDIFF(now, sutv) >= timeout) su = 0;
	return su;
}
...
char *xstrdup(const char *s)
{
	...
	if ((p = strdup(s)) == NULL) die("strdup: %s\n", strerror(errno));
...
{
	if (sel.mode == SEL_EMPTY || sel.ob.x == -1 || sel.alt != IS_SET(MODE_ALTSCREEN)) return 0;

	if (sel.type == SEL_RECTANGULAR) return BETWEEN(y, sel.nb.y, sel.ne.y) && BETWEEN(x, sel.nb.x, sel.ne.x);
...
char *getsel(void)
{
	...
	for (y = sel.nb.y; y <= sel.ne.y; y++) {
		 ...
		 * something is copied from jst and convert '\n' to
...
void stty(char **args)
{
	...
	if ((n = strlen(stty_args)) > sizeof(cmd) - 1) die("incorrect stty parameters\n");
...
int ttynew(const char *line, char *cmd, const char *out, char **args)
{
	...
	if (line) {
		if ((cmdfd = open(line, O_RDWR)) < 0) die("open line '%s' failed: %s\n", line, strerror(errno));
		dup2(cmdfd, 0);
		stty(args);
		return cmdfd;
	}
	...
	if (openpty(&m, &s, NULL, NULL, NULL) < 0) die("openpty failed: %s\n", strerror(errno));
...
{
	if (sel.ob.x == -1 || sel.alt != IS_SET(MODE_ALTSCREEN)) return;
...
void tsetattr(const int *attr, int l)
{
	...
	for (i = 0; i < l; i++) {
		switch (attr[i]) {
		case 0:
			term.c.attr.mode &= ~(ATTR_BOLD | ATTR_FAINT | ATTR_ITALIC | ATTR_UNDERLINE | ATTR_BLINK |
...
void osc_color_response(int num, int index, int is_osc4)
{
	...
	n = snprintf(buf, sizeof buf, "\033]%s%d;rgb:%02x%02x/%02x%02x/%02x%02x\007", is_osc4 ? "4;" : "", num, r, r, g,
		     g, b, b);
	if (n < 0 || n >= sizeof(buf)) {
...
void strhandle(void)
{
	...
	case ']': /* OSC -- Operating System Command */
		...
		case 0:
			...
			return;
		case 1:
			if (narg > 1) xseticontitle(strescseq.args[1]);
			return;
		case 2:
		...
		case 12:
			if (narg < 2) break;
			p = strescseq.args[1];
			if ((j = par - 10) < 0 || j >= LEN(osc_table)) break; /* shouldn't be possible */
...
void tputc(Rune u)
{
	...
	if (u < 127 || !IS_SET(MODE_UTF8)) {
		...
		len = utf8encode(u, c);
		if (!control && (width = wcwidth(u)) == -1) width = 1;
	}
	...
	if (term.esc & ESC_STR) {
		...
		if (strescseq.len + len >= strescseq.siz) {
			 ...
			 * some code to stop the str or esc command, then jst
...
check_control_code:
	...
	if (control) {
		...
		if (IS_SET(MODE_UTF8) && ISCONTROLC1(u)) return;
		tcontrolcode(u);
		/*
		 * control codes are not shown ever
		 */
		if (!term.esc) term.lastc = 0;
		return;
